@extends('app')
@section('title','Lihat Data Barang Keluar')
@section('content')

<div class="row">
    <div class="col-12">
        <div class="card-box">
            <h4 class="font-600 mt-0">{{ $barangKeluar->kd_transaksi }}</h4>
            <p class="text-success text-uppercase m-b-20 font-13">{{
                $barangKeluar->kd_barang }} ({{ $barangKeluar->barang->barang }})</p>

            <p class="text-muted font-13">{{ $barangKeluar->barang->deskripsi }}</p>

            <div class="task-tags m-t-20">
                <h5 class="font-600">Keterangan Barang : </h5>
                <p>Quantity : <span class="badge badge-primary">{{ $barangKeluar->quantity}} </span></p>
            </div>
